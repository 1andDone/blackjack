class HouseRules:
    ...
    def __init__(
        ...
        """
        if min_bet <= 0:
            raise ValueError('Minimum bet at the table must be greater than 0.')
        if max_bet <= min_bet:
            raise ValueError('Maximum bet at the table must be greater than the minimum bet.')
        ...
        if resplit_aces and max_hands <= 2:
            raise ValueError('Maximum hands must be greater than 2 if re-splitting aces is allowed.')
