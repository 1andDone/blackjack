...
def test_init_invalid_min_bet():
    """
    Tests the __init__ method within the Rules class
    when an invalid minimum bet is provided.

    """
    with pytest.raises(ValueError) as e:
        Rules(min_bet=0, max_bet=500)
...
def test_init_invalid_blackjack_payout():
    ...
    """
    with pytest.raises(ValueError) as e:
        Rules(min_bet=10, max_bet=500, blackjack_payout=0)
...
def test_init_invalid_max_hands():
    ...
    """
    with pytest.raises(ValueError) as e:
        Rules(min_bet=10, max_bet=500, max_hands=1)
...
def test_init_invalid_double_after_split():
    ...
    """
    with pytest.raises(ValueError) as e:
        Rules(min_bet=10, max_bet=500, double_down=False, double_after_split=True)
...
def test_init_resplit_aces_invalid_max_hands():
    ...
    """
    with pytest.raises(ValueError) as e:
        Rules(min_bet=10, max_bet=500, resplit_aces=True, max_hands=2)
